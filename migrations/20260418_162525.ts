...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`articles_social_posts\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`platform\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`articles_social_posts_order_idx\` ON \`articles_social_posts\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`articles_social_posts_parent_id_idx\` ON \`articles_social_posts\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`articles\` (
  	...
  	\`author\` text DEFAULT 'Oliver Hartmann',
  	\`reading_time\` numeric DEFAULT 5,
  	\`is_featured\` integer DEFAULT false,
  	\`is_weekend_reading\` integer DEFAULT false,
  	\`meta_title\` text,
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`articles_social_posts\`;`)
